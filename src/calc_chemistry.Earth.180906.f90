...
subroutine calc_chemistry(iBlock)
  ...
  use ModEUV
  use ModSources
  use ModInputs, only: iDebugLevel, UseIonChemistry, UseNeutralChemistry,f107,f107a,UseRocketExhaust
  use ModConstants
     ...
     do iLat = 1, nLats
        ...
        do iAlt = 1, nAlts
           ...
           do while (DtTotal < Dt)
              ...
              !!!!!             -> NO+ + N(4S) + 3.08 eV
              !!!!!             -> O+  + N2 + 1.96 eV
              ! From Fox & Sung (2001) and Scott et al. 1999 quoted in Yonker (2013)

              rr = 1.4e-16*(tn/300.0)**(-0.44)
              ...
              NeutralSources(iN_2D_) = NeutralSources(iN_2D_) + 0.9 * Reaction
              ...
              ChemicalHeatingSubI = &
                   ChemicalHeatingSubI + Reaction * 0.9 * 0.223 + &
                   Reaction * 0.980 * 0.05 + &
                   Reaction * 0.05 * 1.25
             ...
             ! -----------
             ! From Midey & Viggiano (1999) quoted in Yonker (2013)
...
!                   Reaction * 0.9
              ...
              ! NO+ + e -> O + N(2D) + 0.38 eV 
              ! -----------
              ! From Sheehan & St-Maurice (2004) and Hellberg et al. (2003)
              ! quoted in Yonker (2013)

              rr = 3.5e-13*(tn/300.0)**(-0.69)
              ...
              ! -----------
              ! From Fell (1990) quoted in Yonker (2013)
              ...
              ! -----------
              ! From Sander et al. (2006) quoted in Yonker (2013)
              
              rr = 1.5e-17 * exp(-3600.0/tn)
              ...
              ! N(2D) + O2 -> N(4S) + O2(a1Delta,b1Sigma) - 0.234 eV
              ! -----------
              ! Herron (1999) quoted in Yonker (2013)
              ! iN_4S_

              rr = 9.7e-18 * exp(-185.0/tn)
              ...
              if ((UseNeutralConstituent(iN_4S_)) .AND. &
                      (UseNeutralConstituent(iO_1D_))) then

                 NeutralSources(iO_3P_) = &
                      NeutralSources(iO_3P_) + 0.96 * 0.9 * Reaction
                 NeutralSources(iO_1D_) = &
                      NeutralSources(iO_1D_) + 0.96 * 0.1 * Reaction

                 NeutralSources(iN_4S_) = &
                      NeutralSources(iN_4S_) + 0.04 * Reaction
                 NeutralSources(iO2_) = &
                      NeutralSources(iO2_) + 0.04 * Reaction

                 ChemicalHeatingSub = &
                      ChemicalHeatingSub + &
                      0.96 * Reaction * 3.76 * 0.9 + &
                      0.96 * Reaction * 1.80 * 0.1 - &
                      0.04 * Reaction * 0.234
                 ! Heating in last line based on NIST data
                 ...
                 NeutralSources(iNO_)   = NeutralSources(iNO_) + 0.96 * Reaction


              else if (UseNeutralConstituent(iN_4S_)) then
              ...
              else if (UseNeutralConstituent(iO_1D_)) then
