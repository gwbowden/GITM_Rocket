...
subroutine set_inputs
  ...
  do while (.not. IsDone)
     ...
     if (cLine(1:1) == "#") then
        ...
        case ("#CO2COOLING")
           call read_in_logical(UseCO2Cooling, iError)
           if (iError /= 0) then
              write(*,*) 'Incorrect format for #THERMO:'
              write(*,*) ''
              write(*,*) '#CO2COOLING'
              write(*,*) "UseCO2Cooling     (logical)"
              IsDone = .true.
           endif
        ...
        case ("#ELECTRONTHERMALCONDUCTION")
           call read_in_logical(UseImprovedElectronThermalConduction, iError)
           call read_in_real(ElectronThermalConduction_d1, iError)
           call read_in_real(ElectronThermalConduction_d2, iError)
           call read_in_real(ElectronThermalConduction_d3, iError)
           call read_in_real(ElectronThermalConduction_d4, iError)
           call read_in_real(ElectronThermalConduction_d5, iError)
           call read_in_real(ElectronThermalConduction_dt, iError)
           if (iError /= 0) then
              write(*,*) 'Incorrect format for #ELECTRONTHERMALCONDUCTION:'
              write(*,*) ''
              write(*,*) '#ELECTRONTHERMALCONDUCTION'
              write(*,*) "UseImprovedElectronThermalConduction (logical)"
              write(*,*) "ElectronThermalConduction_d1 (Weighting N2: 0.0, real)"
              write(*,*) "ElectronThermalConduction_d2 (Weighting O2: 0.0, real)"
              write(*,*) "ElectronThermalConduction_d3 (Weighting O: 0.0, real)"
              write(*,*) "ElectronThermalConduction_d4 (Weighting He: 0.0, real)"
              write(*,*) "ElectronThermalConduction_d5 (Weighting H: 0.0, real)"
              write(*,*) "ElectronThermalConduction_dt (Weighting Total: 1.0, real)"
           endif

        case ("#NOCOOLINGRATE")
           call read_in_real(k10O, iError)
           if (iError /= 0) then
              write(*,*) 'Incorrect format for #NOCOOLINGRATE:'
              write(*,*) ''
              write(*,*) "#NOCOOLINGRATE"
              write(*,*) "k10O (Rate coefficient for O 1->0: 3.6e-17, real)"
           endif

        case ("#OPLUSFLUX")
           call read_in_logical(UseOplusFlux, iError)
           call read_in_real(PhiDO, iError)
           call read_in_real(PhiNO, iError)
           if (iError /= 0) then
              write(*,*) 'Incorrect format for #OPLUSFLUX:'
              write(*,*) ''
              write(*,*) '#OPLUSFLUX'
              write(*,*) "UseOplusFlux (logical)"
              write(*,*) "PhiDO (Daytime flux: 2.0e12 /s/m/m, real)"
              write(*,*) "PhiNO (Nighttime flux: -2.0e12 /s/m/m, real)"
           endif
        ...
        case ("#IONVELOCITYTIMECONSTANT")
           call read_in_logical(UseIVTC, iError)
           call read_in_real(IVTC, iError)
           if (iError /= 0) then
              write(*,*) 'Incorrect format for #IONVELOCITYTIMECONSTANT'
              write(*,*) ''
              write(*,*) '#IONVELOCITYTIMECONSTANT'
              write(*,*) "UseIVTC                (logical)"
              write(*,*) "IVTC                   (real)"
           endif
...
!!$              write(*,*) "sigmaAlt               (real)"
        ...
        case ("#SAVEPLOTS", "#SAVEPLOT")
           ...
           if (iError /= 0) then
              ...
              write(*,*) '1DTHM, 1DNEW, 1DCHM, 1DCMS, 1DUSR, 3DTHN, 1DTHN.'
              write(*,*) 'DtPlot sets the frequency of output'
              write(*,*) '#SAVEPLOT'
